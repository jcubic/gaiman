...
var keywordList = [
  ...
  "let", "not", "or", "post", "return", "then", "true", "while", "throw", "lambda", 
], keywords = wordObj(keywordList);

var indentWords = wordObj(["def", "for", "while", "until", "then"]);
...
CodeMirror.defineMode("gaiman", function(config) {
  ...
  function tokenBase(stream, state) {
    ...
    if (ch == "`" || ch == "'" || ch == '"') {
      ...
      return chain(readHereDoc(m[1], true), stream, state);
    } else if (ch == "0") {
  ...
  return {
    ...
    indent: function(state, textAfter) {
      ...
      var closed = ct.type == closing[firstChar] ||
        ct.type == "keyword" && /^(?:end|until|else)\b/.test(textAfter);
